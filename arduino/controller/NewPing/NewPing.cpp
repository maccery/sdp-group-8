...
unsigned long NewPing::ping_mm() {
	unsigned long echoTime = NewPing::ping();         // Calls the ping method and returns with the ping echo distance in uS.
#if ROUNDING_ENABLED == false
	return (echoTime / US_ROUNDTRIP_MM);              // Call the ping method and returns the distance in mm (no rounding).
#else
	return NewPingConvert(echoTime, US_ROUNDTRIP_MM); // Convert uS to mm.
#endif
}
...
#if ROUNDING_ENABLED == false
	return (echoTime / US_ROUNDTRIP_MM);              // Convert uS to centimeters (no rounding).
#else
